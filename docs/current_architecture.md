...
- Market scan API: production serves paginated v2 reads through `/api/scan/market/index` and `/api/scan/market/results`; `/api/runtime-config` advertises `v2` so the browser never downloads the full scan. Legacy `GET/POST /api/scan/market` remains as a fallback but the Worker streams the pre-compacted R2 summary text through (`cloudflare/worker_market_legacy.py`) instead of JSON-decoding it, because decoding the multi-megabyte payload inside Pyodide exceeded Worker resource limits and crashed the Worker in production.

...
> This is a historical MVP specification, not an active implementation contract or agent instruction.
> Use `docs/current_architecture.md` for current architecture and operational behavior.
> Any localStorage sections below are legacy MVP notes.

> 版本：v0.1
> 狀態：歷史 MVP 產品與技術規格，不作為目前實作或 agent 指令來源
> 語言：繁體中文
...
| 編號 | 條件 | 預設判斷 |
|---|---|---|
| X1 | 當年度的累計營收年增率需達最新當月份營收年增率的 50% | cumulative_revenue_yoy >= monthly_revenue_yoy * 0.5 |
| X2 | 月營收年增率較上月突然少超過 20 個百分點，且本月年增率未高於 200% | previous_month_revenue_yoy - current_month_revenue_yoy > 20 and current_month_revenue_yoy <= 200 |
...
3. X1 當年度的累計營收年增率未達最新當月份營收年增率的 50% → WARNING 或 EXIT
...
| 一般月份規則 | 春節月份改法 |
|---|---|
| 當年度的累計營收年增率未達最新當月份營收年增率的 50% 可列出場警訊 | 先標記為 SPRING_FESTIVAL_WATCH，不直接 EXIT |

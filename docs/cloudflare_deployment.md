...
The Worker serves paginated v2 market reads with edge cache headers and nothing else. The whole-scan `GET/POST /api/scan/market` payload (several MB) exhausted Python Worker resource limits in production (2026-09), so those routes were retired on 2026-09-15 and now return 404; there is no market API version switch in the config any more, and the only market-read rollback is step 15 above (`wrangler rollback --yes`). The `/api/reports/market` export streams `public/reports/market_scan.csv` and `public/reports/market_scan.md`, which the seed build renders; `market_scan_summary.json` is no longer built or uploaded. The Worker cron and GitHub dispatch are enabled in the committed config (see "Server-Side R2 Seed Rebuild"); `scripts/check_deployment_preflight.py` rejects a config that turns either of them off. The Worker `[cache]` binding is still enabled only through generated full configs from `scripts/render_wrangler_release_config.py`.

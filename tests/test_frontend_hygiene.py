...
def test_frontend_hygiene_current_budget_passes():
    ...
    assert validate_frontend_hygiene(report, max_app_lines=2450, max_inner_html=29) == []


def test_frontend_hygiene_rejects_inner_html_growth():
    report = {
        "appLines": 100,
        "innerHTMLAssignments": 30,
        "insertAdjacentHTMLCalls": 0,
        "hasSharedEscapeHelper": True,
        "emptyStateUsesSharedHelper": True,
    }

    problems = validate_frontend_hygiene(report, max_app_lines=2450, max_inner_html=29)

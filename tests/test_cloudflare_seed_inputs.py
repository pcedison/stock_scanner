...
def _seed_members(
    ...
    members: dict[str, str | bytes] = {
        ...
        "cloudflare_seed/reports/market_scan.csv": "category,stockCode,companyName,status,summary\r\n",
        "cloudflare_seed/reports/market_scan.md": "# 台股市場掃描報告\n",
    }
    members.update(
        {
...
def test_validate_seed_zip_requires_both_static_market_report_exports_when_one_is_present(tmp_path):
    members = _seed_members()
    members.pop("cloudflare_seed/reports/market_scan.csv")

    with pytest.raises(ValueError, match="missing market report exports: cloudflare_seed/reports/market_scan.csv"):
        validate_seed_zip(_zip_from(tmp_path, members))


def test_validate_seed_zip_rejects_a_zip_with_neither_market_report_export(tmp_path):
    # The Worker streams these exports; without them /api/reports/market only serves 503.
    members = _seed_members()
    members.pop("cloudflare_seed/reports/market_scan.csv")
    members.pop("cloudflare_seed/reports/market_scan.md")

    with pytest.raises(ValueError, match="missing market report exports"):
        validate_seed_zip(_zip_from(tmp_path, members))


def test_validate_seed_zip_tolerates_a_legacy_market_scan_summary_member(tmp_path):
    # A zip built by the previous builder still carries the retired summary object.
    members = _seed_members()
    members["cloudflare_seed/market_scan_summary.json"] = json.dumps({"entry": [], "watch": [], "excluded": []})

    assert validate_seed_zip(_zip_from(tmp_path, members))["seedCompanies"] == TEST_SEED_COMPANIES


def test_validate_seed_zip_rejects_a_market_report_csv_with_the_wrong_header(tmp_path):
    members = _seed_members()
    members["cloudflare_seed/reports/market_scan.csv"] = "group,stockCode,companyName,status,summary,rules"

...
def test_validate_seed_zip_still_accepts_a_seed_built_before_the_report_exports_existed(tmp_path):
    # The committed zip is regenerated by the weekly workflow; until then CI validates the
    # previous build, which has neither export. The Worker keeps its fallback for that window.
    members = _seed_members()
    members.pop("cloudflare_seed/reports/market_scan.csv")
